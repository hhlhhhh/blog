	...
	<div>
		<div class="container">
			...
			<el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
				...
				<el-table-column prop="target" label="跳转方式" align="center"></el-table-column>
		...
		<el-dialog title="查看友链详情" v-model="visible1" width="500px" destroy-on-close>
...
import { deleteFriendLinkApi, getFriendLinkList } from '../api/friendLink';
import TableEdit from '../components/friendLink/table-edit.vue';
import TableDetail from '../components/friendLink/table-detail.vue';
...
// 获取表格数据
const getData = async () => {
	const res = await getFriendLinkList(query);
...
const handleDelete = (index: number) => {
	...
	ElMessageBox.confirm('确定要删除吗？', '提示', {
		...
		.then(() => {
			deleteFriendLinkApi(tableData.value[index].id).then(res => {
